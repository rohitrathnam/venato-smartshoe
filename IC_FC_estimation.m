...
TSW=find(gyr_R(:,2)>0.2*max(gyr_R(:,2))); %trusted swing phase
TCS=find(gyr_R(:,2)<0.2*max(gyr_R(:,2))); %candidate stance
len(1)=0;
flag=0;
j=1;
...
for i=1:length(TCS)-1
    ...
    elseif TCS(i+1)~=TCS(i)+1 && len(j)>=50
        flag=0;
        end_CS(j)=TCS(i);
        std_CS(j)=std(gyr_R(begin_CS(j):end_CS(j),2));
        j=j+1;
        len(j)=0;
    elseif TCS(i+1)~=TCS(i)+1 && len(j)<=50
        flag=0;
...
for j=1:length(begin_CS)
    flag=0;
    for i=1:fix(len(j)/2)
        std1(j,i)=std(gyr_R((begin_CS(j)+i-1):(end_CS(j)-i+1),2));
...
for i=1:length(begin_TIC)
    for j=begin_TIC(i):end_TIC(i)
        if norm(acc_R(j,:))>nor_max
            nor_max=norm(acc_R(j,:));
            ind=j;
        end
    end
    IC(i)=find(min(gyr_R((begin_TIC(i):ind),2)))+begin_TIC(i); %index of min angular velocity before acc max
    nor_max=0;
end

% search for final contacts in TFC
for i=1:length(begin_TFC) 
    FC(i)=find(min(acc_R(find(min(gyr_R(begin_TFC(i):end_TFC(i),2)))+begin_TFC(i):end_TFC(i),3)))+find(min(gyr_R(begin_TFC(i):end_TFC(i),2)))+begin_TFC(i);  
end
